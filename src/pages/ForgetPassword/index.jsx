...
export default function ForgetPassword() {
    ...
    const onForget = async () => {
        if ( !userInfo.account || !userInfo.password || !userInfo.email || !userInfo.code) {
            showToast("信息有误，请检查~", true);
            return;
        }
        ...
        UserApi.forget({...userInfo, password: encrypt.encrypt(userInfo.password)}).then((res) => {
            if (res.code === 0) {
                showToast("密码找回成功请登录~");
    ...
    return (
        <CustomBox>
            ...
            <CustomDragDiv className="forget">
                <div style={{width: 300, display: "flex", flexDirection: "column", alignItems: "center"}}>
                    <div style={{fontSize: 24, marginBottom: 50, fontWeight: 600}}>找回密码</div>
                    <div style={{
                        display: "flex",
                    ...
                    <div style={{display: "flex", marginTop: 20}}>
                        <CustomButton width={140} onClick={onForget}>
                            提交
                        </CustomButton>
                    </div>
                </div>
